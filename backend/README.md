...
- LangGraph 模型文本、商品工具进度和最终结果的 SSE 流式协议。
- 商品、模型、LangGraph 和会话模块的自动化测试。

当前边界：LangGraph 运行时已经可以在 Python 中完成真实模型与商品工具的调用
闭环，会话数据格式、内存仓库、普通与 SSE 接口也已经完成，前端可以实时显示模型
文本和商品工具进度。尚未实现数据库持久化、MCP、RAG、订单和售后能力。
...
前端普通对话和商品咨询默认调用聊天接口，包括“你是谁”“你好”等不含商品关键词的
问题；订单、售后和人工服务仍然使用浏览器模拟逻辑。
...
当前共有 116 项后端测试。自动化测试使用模型替身，不会消耗模型额度。真实模型闭环
