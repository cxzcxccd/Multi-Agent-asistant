...
前端聊天仍由浏览器模拟脚本驱动；后端已经完成商品接口和 AI 商品咨询运行时，但
尚未提供 `/api/chat`，所以前后端还没有连接。
...
- 75 项后端测试；会话接口、前端接入、MCP、RAG、订单和售后尚未实现。
...
当前前端数据流是：**用户交互 → 模拟脚本选择场景 → 模拟查询事件 → 更新会话或
申请状态 → 相关页面同步展示**。

当前后端 AI 数据流是：**用户消息 → LangGraph 模型节点 → 商品工具节点 → 模拟商品
数据 → 模型最终回复**。下一步会通过聊天接口把两条数据流连接起来。
...
下一阶段是实现会话数据格式、内存会话服务和 `/api/chat`，再把买家聊天页接入真实
LangGraph 客服运行时。
